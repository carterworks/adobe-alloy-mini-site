...
const createConfigure = ({ logger }: { logger: Logger }): Command<ConfigureArgs> => {
  return {
    ...
    async handler(args: ConfigureArgs) {
      logger.info("Configuring Alloy with datastreamId", args.datastreamId, "and orgId", args.orgId);
...
const createRenderPersonalizations = ({ logger }: { logger: Logger }): Command<{}, APIPersonalization[]> => {
  return {
...
window["alloy"] = function (command: string, ...args: unknown[]) {
